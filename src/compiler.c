...
{
    // Global variables are impliity declared
    ...
    for(int i=current->localCount-1;i>=0;i--)
    {
        ...
        if(identifiersEqual(name,&local->name))
        {
            error("Already variable with this name in tho=is scope.");
        }
    }
    addLocal(*name);
}
...
static void and_(bool cannAssgin)
{
...
static void or_(bool cannAssgin)
{
...
ParseRule rules[] = {
    [TOKEN_LEFT_PAREN]    = {grouping,NULL,PREC_NONE},[TOKEN_RIGHT_PAREN]   = {NULL,NULL,PREC_NONE},
